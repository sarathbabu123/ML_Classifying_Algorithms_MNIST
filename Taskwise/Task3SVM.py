...
# Create an SVM model
# svm_model = SGDClassifier(loss='hinge', alpha=0.0001, max_iter=1000, tol=1e-3, random_state=42)
...
# Train the model
svm_model.fit(x_train_threshold_flattened, y_train)

# Test the model
y_pred = svm_model.predict(x_test_threshold_flattened)
thres_accuracy = accuracy_score(y_test, y_pred)

print("SVM on TEST DATASET")

print(f"SVM 🧪 Thresholded Accuracy: {thres_accuracy:.4f}")

### SVM on box bounding dataset.
...
svm_model.fit(x_train_bound_box_flattened, y_train)

y_pred = svm_model.predict(x_test_bound_box_flattened)
bb_accuracy = accuracy_score(y_test, y_pred)

print(f"SVM 🧪 Bound Box Accuracy: {bb_accuracy:.4f}")

## SVM on box bounding streched dataset.
...
svm_model.fit(x_train_bound_box_streched_flattened, y_train)

y_pred = svm_model.predict(x_test_bound_box_streched_flattened)
bbs_accuracy = accuracy_score(y_test, y_pred)

print(f"SVM 🧪 Bound Box Streched Accuracy: {bbs_accuracy:.4f}")

### SVM on box bounding streched threshold dataset.
...
svm_model.fit(x_train_bound_box_streched_threshold_flattened, y_train)

y_pred = svm_model.predict(x_test_bound_box_streched_threshold_flattened)

bbs_thres_accuracy = accuracy_score(y_test, y_pred)

print(f"SVM 🧪 Bound Box Streched tresholded Accuracy: {bbs_thres_accuracy:.4f}")
# Testing on Train data
### SVM on threshold dataset.

svm_model.fit(x_train_threshold_flattened, y_train)

y_pred = svm_model.predict(x_train_threshold_flattened)
thres_accuracy = accuracy_score(y_train, y_pred)

print("SVM on TRAIN DATASET")

print(f"SVM 🚂 Thresholded Accuracy: {thres_accuracy:.4f}")

### SVM on boundbox dataset.
svm_model.fit(x_train_bound_box_flattened, y_train)

y_pred = svm_model.predict(x_train_bound_box_flattened)
bb_accuracy = accuracy_score(y_train, y_pred)

print(f"SVM 🚂 Bound Box Accuracy: {bb_accuracy:.4f}")

## SVM on boundbox streched dataset.

svm_model.fit(x_train_bound_box_streched_flattened, y_train)

y_pred = svm_model.predict(x_train_bound_box_streched_flattened)
bbs_accuracy = accuracy_score(y_train, y_pred)

print(f"SVM 🚂 Bound Box Streched Accuracy: {bbs_accuracy:.4f}")

### SVM on boundbox streched threshold dataset.
svm_model.fit(x_train_bound_box_streched_threshold_flattened, y_train)

y_pred = svm_model.predict(x_train_bound_box_streched_threshold_flattened)
bbs_thres_accuracy = accuracy_score(y_train, y_pred)

print(f"SVM 🚂 Bound Box Streched threshold Accuracy: {bbs_thres_accuracy:.4f}")
